...
class microring:
    ...
    def split_step(self,plan_fft=False,threads=1,wisdom=None,fastmath=False,Pthreads=1):
        ...
        with tqdm(total=self.Nsnapshots) as pbar:
            ...
            dt = np.complex128(self.dt)
            Dint_arr = np.complex128(self.Dint_arr)
            noise_norm = self.noise_norm
            Δωext = np.complex128(self.Δωext)
            Δωi = np.complex128(self.Δωi)
            Δωtot = np.complex128(self.Δωtot)
            δarr = np.complex128(self.δarr)
            Nsim = self.Nsim
            ################################
